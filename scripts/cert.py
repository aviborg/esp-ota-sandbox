...
def printData(data, showPub = True):
    ...
    for ca in cas:
        with urllib.request.urlopen(ca) as crt:
            print()
            print('// ' + ca)
            root_cert_name = printData(crt.read(), False)
        print()
    
    return root_cert_name if root_cert_name else (name if not showPub else None)
    ...
    with socket.create_connection((hostname, port)) as sock:
        with context.wrap_socket(sock, server_hostname=hostname) as ssock:
            ...
            if name:
                ...
                print()
            root_cert = printData(ssock.getpeercert(binary_form=True))
            if name and root_cert:
                print()
                print('// Root CA certificate alias for code compatibility')
                print('#define {}_cert cert_{}'.format(name, root_cert))
